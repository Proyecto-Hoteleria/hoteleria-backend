...
export const isAdmin = async (req, res, next) => {
    try {
        let { user } = req
        if (!user || user.role !== 'ADMIN') return res.status(403).send({ message: `You dont have access | email: ${user.email}` })
        next()
    } catch (err) {
        console.error(err)
...
export const isOneSelf = async (req, res, next) => {
    ...
    if (!user || user.role !== 'ADMIN') {
        if (user.id !== id) return res.status(403).send({ message: `You dont have access | email: ${user.email}` })
        next()
    } else {
        next()
    }
