...
export const getAllRooms = async (req, res) => {
    try {
        let rooms = await Room.find().populate('id_hotel');
...
export const getRoomById = async (req, res) => {
    try {
        let { id } = req.params;
        let room = await Room.findById(id).populate('id_hotel');
        if (!room) {
...
export const updateRoom = async (req, res) => {
    try {
        ...
        let updatedRoom = await Room.findByIdAndUpdate({ _id: id }, data, { new: true }).populate('id_hotel');
        if (!updatedRoom) {
            return res.status(404).send({ message: 'Room not found.' });
        }
